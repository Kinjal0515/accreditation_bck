...
class DashboardController extends Controller
{
    public function BookingCounts($id)
    {
        ...
        if ($isAdmin) {
            $onlineBookingsCount = Booking::count();
            $agentBookingsCount = Agent::count();
            $sponsorBookingsCount = SponsorBooking::count();
            // $agentBookingsCount = Agent::whereNotNull('deleted_at')->count();
            // return response()->json($agentBookingsCount);
            ...
            $offlineBookings = $posBookingsCount + $agentBookingsCount + $sponsorBookingsCount;
    ...
    public function calculateSale(Request $request)
    {
        ...
        $cacheKey = $this->generateCacheKey($isAdmin, $isOrganizer, $isPOS, $isAgent, $isSponsor, $loggedInUser->id);

        $data = Cache::remember($cacheKey, 60, function () use ($request, $isAdmin, $isOrganizer, $isAgent, $isSponsor, $isPOS, $loggedInUser) {
            ...
            if ($isAdmin) {
                ...
                return array_merge(
                    $this->calculation($easebuzzTotalAmount, $instamojoTotalAmount, $onlineBookings, $offlineBookings, $sponsorBookings, $posTotals, 'Admin'),
                ...
                return array_merge(
                    $this->calculation($easebuzzTotalAmount, $instamojoTotalAmount, $onlineBookings, $offlineBookings, $sponsorBookings, $posTotals, 'Organizer'),
                    ['weeklyData' => $weeklyData],
                    ['weeklyDataForCF' => $weeklyDataForCF]
                );
            } elseif ($isPOS) {
                ...
                $posTotals = PosBooking::where('user_id', $loggedInUser->id)
                    ->selectRaw('
                        SUM(amount) as total_base_amount,
                        SUM(CASE WHEN created_at BETWEEN ? AND ? THEN amount ELSE 0 END) as today_base_amount,
                        SUM(convenience_fee) as total_convenience_fee,
                        SUM(CASE WHEN created_at BETWEEN ? AND ? THEN convenience_fee ELSE 0 END) as today_convenience_fee
                    ', [$todayStart, $todayEnd, $todayStart, $todayEnd])
                    ->first();
                $weeklyData = $this->getWeeklySalesDataForPOS($loggedInUser->id);
                $weeklyDataForCF = $this->getWeeklyCFDataForPOS($loggedInUser->id);
                return array_merge(
                    $this->calculation($onlineBookings, $offlineBookings, $posTotals, 'POS'),
                    ['weeklyData' => $weeklyData],
                    ['weeklyDataForCF' => $weeklyDataForCF]
                );
            } elseif ($isAgent) {
                ...
                return array_merge(
                    $this->calculation(0, 0, $onlineBookings, $offlineBookings, $posTotals, 'Agent'),
                ...
                return array_merge(
                    $this->calculation(0, 0, $onlineBookings, $offlineBookings, $sponsorBookings, $posTotals, 'Sponsor'),
        ...
        if ($isAgent) {
            ...
            return response()->json([
                ...
                'posTodayAmount' => $data['posTodayAmount'],
    ...
    private function generateCacheKey($isAdmin, $isOrganizer, $isPOS, $isAgent, $isSponsor, $userId)
    {
        if ($isAdmin) {
            return "admin_sales";
        }
    ...
    private function calculation($easebuzzTotalAmount, $instamojoTotalAmount, $onlineBookings, $offlineBookings, $sponsorBookings, $posTotals, $role)
    {
        if ($role == 'Agent') {
            return [
    ...
    public function getDashboardSummary($type, Request $request)
    {
        ...
        switch ($type) {
            ...
            case 'pos':
                ...
                // $totalTickets = PosBooking::whereBetween('created_at', [$startDate, $endDate])->sum('quantity');
                // break; $query = PosBooking::whereBetween('created_at', [$startDate, $endDate]);
